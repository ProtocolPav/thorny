    ...
    async def build(cls, guild: discord.Guild) -> Guild:
        async with pool.acquire() as conn:
            ...
            activity = await conn.fetch("""
                                        SELECT SUM(playtime) as playtime FROM thorny.activity
                                        INNER JOIN thorny.user ON thorny.activity.thorny_user_id = thorny.user.thorny_user_id
                                        WHERE DATE_PART('month', connect_time) = DATE_PART('month', NOW())
	                                    AND DATE_PART('year', connect_time) = date_part('year', NOW())
