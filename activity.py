...
def write_log(status: str, datetime, ctx):
    ...
    ReadFile = open(f'text files/activity_{date.lower()}21.json', 'a')
    if os.path.getsize(f'text files/activity_{date.lower()}21.json') == 0:
        ReadFile.write('[]')
    ReadFile = open(f'text files/activity_{date.lower()}21.json', 'r+')
    ...
    WriteFile = open(f'text files/activity_{date.lower()}21.json', 'w')
...
def profile_update(ctx_author, value=None, key1=None, key2=None):
    ...
    if profile[f'{ctx_author.id}'].get('fields') is None:
